...
String HTTPSRedirect::getLineStr(){ // Function for reading
  return lineStr;
}
...
void HTTPSRedirect::fetchBodyUnChunked(unsigned len){
  ...
  while ((connected()) && (len > 0)) {
    ...
    {
      Serial.println(line);
      Serial.print(" tuka neshto ot men 256");
    }
...
void HTTPSRedirect::fetchBodyChunked(void){
  ...
  while (connected()){
    ...
    while (chunkSize > 0){
      ...
      {
//        Serial.println(line);
//        Serial.print(" tuka neshto ot men 290 " );
     //   lineStr = line;
        lineStr = line;
        getLineStr();
        Serial.print(line);
...
void HTTPSRedirect::fetchBodyRaw(void){
  ...
  while (connected()){
    ...
    {
      Serial.println(line);
      Serial.print(" tuka neshto ot men 470");
    }
